        ...
        if self.nb == 10**9:
            ...
            if self.dtype == "uint8":
                with open(outfile, "rb") as f:
                    n, d = np.fromfile(f, count=2, dtype=np.int32)
                    out = open(outfile.replace("u8bin", "fbin"), "wb")
                    np.array([n, d], dtype=np.uint32).tofile(out)
                    bs = int(n/10)
                    bs = bs * d
                    for i in range(10):
                        data = np.fromfile(f, dtype=np.uint8, count = bs)
                        data.astype(np.float32).tofile(out)
                os.remove(outfile)
...
class Deep1BDataset(DatasetCompetitionFormat):
    def __init__(self, nb_M=1000):
        ...
        self.basedir = os.path.join(BASEDIR, "deep")
